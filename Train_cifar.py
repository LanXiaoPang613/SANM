...
def attention_erase_map(images, outputs, gmmweight, target_mask):
    ...
    for i in range(img.size(0)):
        for attempt in range(1000000000):
            ...
            target_area =  random.uniform(sl, sh) *area + 0.2 * (1-gmmweight[i])
...
def train(epoch,net,net2,optimizer,labeled_trainloader,unlabeled_trainloader):
    ...
    for batch_idx, (inputs_xp, inputs_xp2, inputs_xs1, inputs_xs2, inputs_x, inputs_x2, labels_x, truelab, w_x) in enumerate(labeled_trainloader):      
        try:
            inputs_up, inputs_up2, inputs_us1, inputs_us2,inputs_u,inputs_u2,w_u = unlabeled_train_iter.next()
        except:
            unlabeled_train_iter = iter(unlabeled_trainloader)
            inputs_up, inputs_up2, inputs_us1, inputs_us2,inputs_u,inputs_u2,w_u = unlabeled_train_iter.next()                 
        ...
        loss = Lx + lamb * Lu  + penalty + Lrecon*0.2
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def create_model(dataset='cifar100', num_classes=100):
